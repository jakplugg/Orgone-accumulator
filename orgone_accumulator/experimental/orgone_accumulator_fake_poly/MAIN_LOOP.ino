void loop() {
  ...
  if (ARC == 4) {
    //  Serial.print(o1[0].phase_increment);
    //   Serial.print(" 0  ");
    //    Serial.print(o1[1].phase_increment);
    Serial.print("   diff =");
    Serial.print(abs(inCVraw - inCVck[0]));
    Serial.print("   rr=");
    Serial.println(rr);
  }
  ...
  if (loopReset == 1)goto evilGoto;
  inputConverter[rr] = inputVOct * conf_TuneMult; //<-----------------------------number is MASTER TUNING also affected by ISR speed
...
evilGoto:
  ...
  //----------------------------------------------------------------------
    ...
    switch (FX) {
            ...
            case 7:
              ASSIGNINCREMENTS_TWANG();//spectral
              break;

      default:
        ASSIGNINCREMENTS();//it is in
    }
