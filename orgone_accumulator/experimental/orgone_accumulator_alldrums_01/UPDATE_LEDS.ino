...
void OFF_prog_LEDS() {
  ...
  //pinMode(LED_roundrobinOn, INPUT); 
...
void UPDATE_prog_LEDS() {
  if (LED_MCD > 0) {
    switch (ARC + 1) {
      ...
      case 5:
        ...
        if (bitRead(microByte, (1 - (FX >> 3)))) {
          ...
          //if (FX == 1) {pinMode(LED_roundrobinOn, OUTPUT);} 
    ...
    switch (ARC + 1) {
      case 1:
        if(TUNELOCK_SWITCH == 0 || IsHW2)digitalWriteFast(LED_roundrobin, tuneLockOn); //hardware2 has separate button
        else digitalWriteFast(LED_roundrobin, roundrobinOn ^ gateState);
        break;
      case 2:
      ...
      case 8:        
        break;
      case 9:
      digitalWriteFast(LED_roundrobinOn, roundrobinOn);
        break;
